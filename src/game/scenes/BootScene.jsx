...
export default class BootScene extends Phaser.Scene {
    ...
    create() {
        ...
        playButton.on("pointerdown", () => {
            ...
            const confirmBg = this.add.rectangle(this.sys.game.config.width / 2, 400, 600, 300, 0x000000, 1).setOrigin(0.5);
            const confirmText = this.add.text(this.sys.game.config.width / 2, 370, "Start the timer and begin?", {
                fontSize: "28px",
                color: "#fff",
                align: "center"
            }).setOrigin(0.5);
            const yesBtn = this.add.text(this.sys.game.config.width / 2 - 80, 430, "Yes", {
                fontSize: "32px",
                color: "#00ff00",
                backgroundColor: "#222",
                padding: { left: 20, right: 20, top: 10, bottom: 10 }
            }).setOrigin(0.5).setInteractive({ useHandCursor: true });
            const noBtn = this.add.text(this.sys.game.config.width / 2 + 80, 430, "No", {
                fontSize: "32px",
                color: "#ff0000",
                backgroundColor: "#222",
                padding: { left: 20, right: 20, top: 10, bottom: 10 }
